package com.bj.hmxxteacher.widget;
...
import com.bj.hmxxteacher.zzautolayout.AutoRelativeLayout;
import com.bj.hmxxteacher.zzautolayout.utils.AutoUtils;
import com.bj.hmxxteacher.zzautolayout.utils.L;
 ...
 */
public class MixtureTextView extends AutoRelativeLayout {
    ...
    public MixtureTextView(Context context, AttributeSet attrs) {
        ...
        //just for text
        if (mText == null) {
            mText = getResources().getString(R.string.mixture_textview_default_text);
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mTextPaint.setTextSize(AutoUtils.getPercentWidthSizeBigger(mTextSize));
        L.e("onMeasure()：" + mTextPaint.getTextSize() + "?????");
    ...
    private boolean tryDraw(Canvas canvas) {
        ...
        for (int i = 0; i < destRects.size(); i++) {
            ...
            layoutLineCount = lineCount = layout.getLineCount() < lineCount ? layout.getLineCount() : lineCount;
            if (!kidding) {
                canvas.save();
...
//                mTextSize = size;
                mTextSize = AutoUtils.getPercentWidthSize(size);
                L.e("字体大小：" + mTextSize + "px");
                break;
            case TypedValue.COMPLEX_UNIT_DIP:
                mTextSize = dp2px(size);
                L.e("字体大小：" + mTextSize + "dp");
                break;
            case TypedValue.COMPLEX_UNIT_SP:
                mTextSize = sp2px(size);
                L.e("字体大小：" + mTextSize + "sp");
                break;
        }
    ...
    // 设置是否隐藏所有图片
    public void hideImage(boolean ishide) {
        this.isHideImage = ishide;
    }
