...
public class BGAProgressBar extends ProgressBar {
    ...
    private boolean mIsHiddenPercent;
    ...
    protected void initAttr(int attr, TypedArray typedArray) {
        ...
        } else if (attr == R.styleable.BGAProgressBar_bga_pb_isHiddenPercent) {
            mIsHiddenPercent = typedArray.getBoolean(attr, mIsHiddenPercent);
        }
    }
    ...
    private void calculateTextWidthAndHeight() {
        if (mIsHiddenPercent) {
            // mText = String.format("%d", getProgress());
        } else {
