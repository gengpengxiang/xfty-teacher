    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_UP:
                float sshiftX = Math.abs(currentX - downX);
                float sshiftY = Math.abs(currentY - downY);
                float ddx = currentX - lastEventX;
                float ddy = currentY - lastEventY;
                lastEventY = currentY;
                if (isTop()) {
                    if (sshiftY > sshiftX && sshiftY > touchSlop) {
                        int height = (int) (headerParams.height + ddy / sensitive + 0.5);
                        if (height <= headerHeight) {
                            height = headerHeight;
                            isZooming = false;
                        } else {
                        ...
                        headerParams.height = height;
                        headerView.setLayoutParams(headerParams);
                        if (pullZoomListener != null && (headerParams.height - headerHeight) > (headerHeight / 2)) {
                            pullZoomListener.onUpToZoom();
                        }
                    }
                }
