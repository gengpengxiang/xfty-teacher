...
import com.bj.hmxxteacher.adapter.ReasonEditAdapter;
...
import com.bj.hmxxteacher.dialog.AddReasonAlertDialog;
import com.bj.hmxxteacher.dialog.CancelConfirmAlertDialog3;
import com.bj.hmxxteacher.dialog.CommendReasonInfo;
import com.bj.hmxxteacher.utils.DensityUtils;
...
public class ReasonEditActivity extends BaseActivity {
    ...
    private String type,banji_type;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        banji_type = getIntent().getStringExtra("banji_type");
        ...
        mAdapter.setOnMyItemClickListener(new ReasonEditAdapter.OnMyItemClickListener() {
            ...
            public void onClick(View view, int position) {
                if (mDataList.size() > 1) {
                    ...
                    T.showShort(ReasonEditActivity.this, "评价理由不能为空");
                }
            }
...
//        }
    ...
    private void getCommendReason() {
        Observable.create(new ObservableOnSubscribe<List<CommendReasonInfo>>() {
            ...
            public void subscribe(@NonNull ObservableEmitter<List<CommendReasonInfo>> e) throws Exception {
                ...
                String result = mService.getCommendReasonFromAPI2(teacherPhoneNumber,type,banji_type);
                Log.e("理由结果",result);
    ...
    private List<CommendReasonInfo> parseCommendReasons(String result) throws Exception {
        ...
        if (!StringUtils.isEmpty(result)) {
            ...
            for (int i = 0; i < resultArray.length(); i++) {
                ...
                String reasonID = reasonObj.optString("code");
                String reasonTitle = reasonObj.optString("liyou");
    ...
    private void addNewReason(final String reasonName) {
        Observable.create(new ObservableOnSubscribe<String[]>() {
            ...
            public void subscribe(@NonNull ObservableEmitter<String[]> e) throws Exception {
                LmsDataService mService = new LmsDataService();
                String[] result = mService.addTeacherCommendReasonFromAPI(teacherPhoneNumber, reasonName,type);
                e.onNext(result);
            }
        }).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
